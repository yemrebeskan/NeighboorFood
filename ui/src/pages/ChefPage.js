...
function ChefPage() {
  ...
  async function getChef() {
    ...
    const res = await axios.get(
      `https://neighboorfood-s5im.onrender.com/api/v1/chefs/${id}`
    )
    ...
    const menuRes = await axios.get(
      `https://neighboorfood-s5im.onrender.com/api/v1/chefs/${id}/menu`
    )
    ...
    const reviewResp = await axios.get(
      `https://neighboorfood-s5im.onrender.com/api/v1/reviews/user/${chefData._id}`
    )
